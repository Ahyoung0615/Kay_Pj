...
<body>
	<div id="container">
		...
		<form>
			<input type="submit" value="삭제" id ="allChk" style="margin-right: 20px;" onclick="del(event)">		
			<table class="table table-hover">
				...
				<tbody>
					<c:if test="${fn:length(list) == 0}">
						<tr>
							<td colspan="5" style="color: blue; font-size: 14px; text-align: center;">-- 가입된 회원이 없습니다 --</td>
						</tr>
					</c:if>
					<c:if test="${fn:length(list) != 0}">
						<c:forEach var="dto" items="${list}" varStatus="vs">
							<tr>
								...
								<td>${dto.name}</td>
								<td>${dto.phone}</td>
								<td>${dto.auth eq "ADMIN" ? "관리자" : "사용자"}</td>
								<td>${dto.enable eq "Y" ? "사용중" : dto.enable eq "N" ? "탈퇴" : "대기중"}</td>
							</tr>
						</c:forEach>
						</c:if>
				</tbody>
